<?php
    ...
    {
        $number = $_POST['num'];
        $query="SELECT * FROM Должности WHERE Код_должности='$number'";
    ...
    {
        $id = $_POST['id'];
        $query = "DELETE FROM Должности WHERE Код_должности='$id'";
        $result = $mysqli->query($query);
    }
    
    if (isset($_POST['name']) && isset($_POST['description']) && isset($_POST['salary']) && isset($_POST['add']))
    {
        $name = $_POST['name'];
        $description = $_POST['description'];
        $salary = $_POST['salary'];
        $query = "INSERT INTO Должности (Наименование, Описание, Оклад) VALUES ('$name', '$description', '$salary')";
        $result = $mysqli->query($query);
    }
        
    if (isset($_POST['name']) && isset($_POST['description']) && isset($_POST['salary']) && isset($_POST['update']) && isset($_POST['id']))
    {
        $id = $_POST['id'];
        $name = $_POST['name'];
        $description = $_POST['description'];
        $salary = $_POST['salary'];
        $query = "UPDATE Должности SET Наименование='$name', Описание='$description', Оклад='$salary' WHERE Код_должности='$id'";
        $result = $mysqli->query($query);
    }
    ...
    if (isset($_POST['selectCol']) && isset($_POST['selectSort']) && isset($_POST['search']) && isset($_POST['searchButton']))
    {
        ...
        $query = "SELECT * FROM Должности WHERE Наименование LIKE '%$search%' ORDER BY $selectCol $selectSort";
    }
    
    else $query = "SELECT * FROM Должности";
	...
	<div id="wrapper">
	    ...
	    <div id="page-wrapper">
	        <section id="services" class="servicer-section section-padding">
                	        ...
                	        <form class="searchForm" action="positions.php" method="post">
		            Сортировать по
                    <select class="searchSet" name="selectCol">
                        <option value="Код_должности">Код должности</option>
                    ...
                    Поиск по наименованию должности
        	    ...
        	    <table class="table">
                    ...
                    <form action="positions.php" method="post">
                        <input class="updateButton" type="button" name="' .$line['Код_должности']. '" value="Изменить">
                        <input type="hidden" name="id" value="' .$line['Код_должности']. '">
                ...
                echo '</table>
                <form class="addForm" action="positions.php" method="post">
                    <p class="form_title">Добавить</p>
                    <div class="form_row">
        	            Наименование <input type="text" name="name" maxlength="50" pattern="^[А-Яа-яЁё\s]+$">
                    </div>
                    <div class="form_row">
        	            Описание <input type="text" name="description" maxlength="150">
                    </div>
                    <div class="form_row">
        	            Оклад <input type="text" name="salary" maxlength="50">
                    </div>
                ...
                <form class="updateForm" action="positions.php" method="post">
