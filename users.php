<?php
    ...
    {
        $number = $_POST['num'];
        $query="SELECT * FROM Пользователи WHERE Код_пользователя='$number'";
    ...
    {
        $id = $_POST['id'];
        $query = "DELETE FROM Пользователи WHERE Код_пользователя='$id'";
        $result = $mysqli->query($query);
    }
    
    if (isset($_POST['login']) && isset($_POST['password']) && isset($_POST['add']))
    {
        $login = $_POST['login'];
        $password = $_POST['password'];
        $query = "INSERT INTO Пользователи (Логин, Пароль) VALUES ('$login', '$password')";
        $result = $mysqli->query($query);
    }
        
    if (isset($_POST['login']) && isset($_POST['password']) && isset($_POST['update']) && isset($_POST['id']))
    {
        $id = $_POST['id'];
        $login = $_POST['login'];
        $password = $_POST['password'];
        $query = "UPDATE Пользователи SET Логин='$login', Пароль='$password' WHERE Код_пользователя='$id'";
        $result = $mysqli->query($query);
    }
    ...
    if (isset($_POST['selectCol']) && isset($_POST['selectSort']) && isset($_POST['search']) && isset($_POST['searchButton']))
    {
        ...
        $query = "SELECT * FROM Пользователи WHERE Наименование LIKE '%$search%' ORDER BY $selectCol $selectSort";
    }
    
    else $query = "SELECT * FROM Пользователи";
	...
	<div id="wrapper">
	    ...
	    <div id="page-wrapper">
	        <section id="services" class="servicer-section section-padding">
                	        ...
                	        <form class="searchForm" action="users.php" method="post">
		            Сортировать по
                    <select class="searchSet" name="selectCol">
                        <option value="Код_пользователя">Код услуги</option>
                        <option>Логин</option>
        	    ...
        	    <table class="table">
                    ...
                    <form action="users.php" method="post">
                        <input class="updateButton" type="button" name="' .$line['Код_пользователя']. '" value="Изменить">
                        <input type="hidden" name="id" value="' .$line['Код_пользователя']. '">
                ...
                echo '</table>
                <form class="addForm" action="users.php" method="post">
                    <p class="form_title">Добавить</p>
                    <div class="form_row">
        	            Логин <input type="text" name="login" maxlength="50">
                    </div>
                    <div class="form_row">
        	            Пароль <input type="text" name="password" maxlength="150">
                    </div>
                ...
                <form class="updateForm" action="users.php" method="post">
                        ...
                        $(document).ready(function(){
                            $(".updateButton").click(function(){
                                ...
                                $.post(\'users.php\',
                                    ...
                                    $(".updateForm input[name=\"id\"]").attr("value", msg["Код_пользователя"]);
                                    $(".updateForm input[name=\"login\"]").attr("value", msg["Логин"]);
                                    $(".updateForm input[name=\"password\"]").attr("value", msg["Пароль"]);
